...
require("pacman")
p_load("tidyverse","rvest","rio","skimr","caret","ggplot2","stargazer","boot")
...
GEIHf$lningresoh <- log(GEIHf$inghora)

#Edad al cuadrado
GEIHf$age2 <- I(GEIHf$age^2)

reg1 <- subset (GEIHf, select = c("lningresoh","age","age2"))
view(reg1)

#Regresión lineal
reg_lin <- lm(lningresoh ~ age + age2, data = GEIHf)
...
p_ingh<- 8.080+0.031* reg1$age - 0.0003* reg1$age2
p_ingh <- c(p_ingh)
